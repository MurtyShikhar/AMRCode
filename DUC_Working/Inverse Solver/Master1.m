...
%             chi_0(i) = temp;
%         end;
           if( real(chi_0(i))<0 || imag(chi_0(i))<0)
			chi_0(i)=0;
		end;
...
cost = cost_function()
[g_n, v_n] = UpdateContrastSource1(g_0, v_0, N);
cost = cost_function()
...
while (cost > 0.001)
    [g_n, v_n] = UpdateContrastSource1( g_n, v_n, N);
    UpdateContrast(N,0);

     % uncomment this to run CSI with positive chi (both real and imag)
     
      for i = 1:N*N
		% below lines are default
        % if(real(chi(i))<0)
        %    chi(i)=1i*imag(chi(i));
        %  end;
        %  if (imag(chi(i)) < 0)
        ...
        % alternate approach by Uday on 08.07
        if( real(chi(i))<0 || imag(chi(i))<0)
			chi(i)=0;
		end;
