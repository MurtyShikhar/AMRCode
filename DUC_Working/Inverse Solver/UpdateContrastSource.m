...
%change
G_s_star = aux_GS_star * dat_error;

%Calculate G_D*
chi_obj_mult = zeros(N*N, M);
for i=1:N*N
    %change
    chi_obj_mult(i,:) = conj(chi(i)).*obj_error(i,:);
end

%change
G_d_star = aux_GD_star* chi_obj_mult;

%Calculate g_n which is N^2 x M mtrix
%g_n = (-1*eta_S) .* G_s_star - eta_D .* (obj_error - G_d_star);

%2008 paper
g_n = (-2*eta_S) .* G_s_star - 2 * eta_D .* (obj_error - G_d_star);
...
for i = 1:M
    %sum_numer = sum_numer - real(dot(g_n(:,i), v_n(:,i)));

    %2008 paper
    sum_numer = sum_numer + eta_S*dot(dat_error(:,i), mat_1(:,i) )  + eta_D*dot(obj_error(:,i) ,v_n(:,i)- mat_2(:,i));
...
alpha_n = real(sum_numer/sum_denom)

%Update contrast source
w = w  + alpha_n.*v_n;   %%% move in the conjugate direction
